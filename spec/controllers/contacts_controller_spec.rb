...
describe 'Contacts Controller', type: :request do
  ...
  describe 'POST #create' do
    ...
    context 'when valid data' do
      before do
        post '/en/contacts', params: { contact: data }
      end
    ...
    context 'when invalid data' do
      it 'not create a new contact' do
        expect { post '/en/contacts', params: { contact: { data: 'data' } } }.not_to change(Contact, :count)
      end

      it 'will render new' do
        post '/en/contacts', params: { contact: { data: 'data' } }
  ...
  describe 'PATCH #update' do
    ...
    it 'will set variable' do
      patch "/en/contacts/#{contact.id}", params: { contact: { data: '+380505555559' } }
    ...
    context 'when invalid data' do
      before { patch "/en/contacts/#{contact.id}", params: { contact: { data: '' } } }
  ...
  context 'when destroy' do
    ...
    it 'change count in db' do
      expect { delete "/en/contacts/#{contact.id}" }.to change(Contact, :count).by(-1)
    end

    it 'redirects to all organizations' do
      delete "/en/contacts/#{contact.id}"
